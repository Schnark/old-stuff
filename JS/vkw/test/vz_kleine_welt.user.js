...
// Copyright (C) 2009-2010 Michael Müller
// Email: <Vorname><Unterstrich><Nachname mit aufgloestem Umlaut><Pi mit 4 Nachkommastellen und Unterstrich statt Komma><bei>gmx<Punkt>de
// Profil: <http://www.studivz.net/Profile/OyYe53zJM5Ixupo6skCfl5gRCSc8DJw0GOTr81ZZdzE/>
//
...
// - irgeneine Seite finden, auf der man die Freunde nicht nur in 15er Packungen auslesen kann (<formkey=a7b33aa1b2ce28d0957987d29801310f0702dae170e8a03773f48b7922fab5dd53b1a9d65c3ba9dbfb70f00aed5d1c1205dc6d247edc31c700422b44b668c832d00016c7242e2a2645a4e24031b807f091bd6731d8763d7a45edd33b41855c0ed1d33efbd2a53bcbb850b9aa43a30f3b&amp;iv=39462909e401250a95ceb1801f19a6f9"> sieht doch kryptisch genug aus)
...
var max_profile, max_freunde, max_http, max_dauer;                     //maximale Anzahl an Abfragen/Zeitdauer
...
repeat: do {
    ...
    for (var k = k_min[p] + 1; k <= k_max[p]; k++) {                   //von der kleinsten bis zur größten noch nicht untersuchten Person
        ...
        forR: for (var i = 0; i < anzahl; i++) {                       //jeden Freund untersuchen
            ...
            stat_profile++;
            if (max_profile > 0 && stat_profile > max_profile) { abbruch = 1; abbruchgrund = "Die maximale Anzahl an Profilen wurde erreicht."; }
            if (max_dauer > 0 && max_dauer * 1000 < timestamp() - startzeit) { abbruch = 1; abbruchgrund = "Der Abbruch erfolgte wegen Zeitüberschreitung."; }
...
var statistik = "<small>Bei der Suche wurden " + stat_profile + " Personen untersucht, von " +
    stat_freunde + " wurden dabei alle Freunde bestimmt, was zu " + stat_http + " Anfragen führte.";
    + "Sie dauerte " + Math.round((timestamp()-startzeit)/1000) + " Sekunden.</small>";
...
text += "Maximale Dauer (in Sekunden), die die Suche dauern darf: " +
        "<input type='text' size='3' maxlength='3' id='vkw_maxdauer' value='" + max_dauer + "'/><br />";
text += "</div>";
...
text += "&copy; 2009-2010 <a href='" + url + "Profile/OyYe53zJM5Ixupo6skCfl5gRCSc8DJw0GOTr81ZZdzE/'>Michael Müller</a><br />"; //bzw. "&copy; 2009-2010 Michael M.<br />"
...
wert = document.getElementById("vkw_maxdauer").value;
max_dauer = (isNaN(wert)) ? 0 : wert;
...
GM_setValue(gm_praefix + "maxdauer", max_dauer);
...
GM_deleteValue(gm_praefix + "maxdauer");
...
antwort += "\nDauer: " + Math.round((timestamp()-startzeit)/1000);
alert(antwort);
}
...
//profId = document.getElementById("Grid-Navigation-Main");
 //if (profId) {
 //   profId = profId.getElementsByTagName("li")[1].getElementsByTagName("a")[0].href;
 //   profId = profId.substring(profId.indexOf("Profile/") + 8, profId.length - 8);
 //} else { profId = ""; }
 ...
 max_dauer = GM_getValue(gm_praefix + "maxdauer", 0);
